...
@section('title', 'Processzor módosítása')

@section('content')
    <h1 class="text-2xl font-semibold mb-4">Processzor módosítása</h1>

    <form action="{{ route('processors.update', $processor) }}" method="POST" class="space-y-4 max-w-md">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <input type="text" name="gyarto"
                   value="{{ old('gyarto', $processor->gyarto) }}"
        ...
        <div>
            ...
            <input type="text" name="tipus"
                   value="{{ old('tipus', $processor->tipus) }}"
                   class="w-full px-3 py-2 rounded bg-zinc-800 text-white">
            @error('tipus')
                <div class="text-red-400 text-sm mt-1">{{ $message }}</div>
            @enderror
